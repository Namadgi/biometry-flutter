...
### Added
- `faceMatch()` now detects the content-type of the reference image from its file extension (`image/jpeg` for `.jpg`/`.jpeg`, `image/png` otherwise), replacing the previous hardcoded `application/png`.
- `assertConsent()` is now public (renamed from `_assertConsent()`), allowing callers to verify consent state explicitly before performing biometric operations.

### Changed
- **Consent history API:** `ConsentHistoryResult.consent` and `ConsentHistoryResult.storageConsent` are now nullable (`ConsentRecord?`) to match the API — either field can be null when only one consent type has been recorded. Callers that assumed non-null values must add null checks.
- **getConsentHistory():** A `404` response now returns a `ConsentHistoryResult` with both consent fields `null` instead of throwing. Non-404 errors still throw. Parse/format failures now throw with an "Unexpected consent history response format" message.
- **assertConsent():** Guards against a null `consent` field before reading `isConsentGiven`.
- Removed redundant internal `_assertConsent()` pre-checks from `enrolFace()`, `enrolVoice()`, `docAuth()`, `processVideo()`, and `faceMatch()`.
- Added documentation comments to `ConsentHistoryResult`, `ConsentRecord`, and `ConsentHistoryEntry` model fields.

### Fixed
- Consent history parsing no longer throws a `TypeError` when the API returns `null` for `consent` or `storage_consent`.
- `_processFaceImage()` null-checks the response data before accessing its keys.
